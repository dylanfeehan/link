...
struct message_node* create_message_node(const char __user * from, const char __user * message) {
	...
	if(new_message->from == NULL) return NULL;

	new_message->message = kmalloc(sizeof(char) * strlen(message), GFP_KERNEL);
	if(new_message->message == NULL) return NULL;

	if(copy_from_user((void*)(new_message->from), (char *)from, strlen(from)) > 0) return NULL;
	if(copy_from_user((void*)(new_message->message), (char *)message, strlen(message)) > 0) return NULL;
...
SYSCALL_DEFINE3(cs1550_send_msg, const char __user *, to, const char __user *, msg, const char __user *, from) 
{
	...
	if(strlen(to) == 0 || strlen(msg) == 0) return -1;

	struct user_node * user = find_user_node(to);
	struct message_node * new_message = create_message_node(from, message);
	if(new_message == NULL) return -1; // create messgae node throws this to here

	if(user) { 
		append_message_node(user, new_message);
		user->message_count = user->message_count + 1;
	}
	else {
		...
		new_user->user = kmalloc(sizeof(char) * strlen(to), GFP_KERNEL);
		if(new_user->user == NULL) return -1;

		if(copy_from_user(((void*)new_user->user), to, strlen(to)) > 0) return -1;

		append_message_node(new_user, new_message);
		insert_user_node(new_user);
...
{
	/*  GET MESSAGE */
	if(to == NULL || message == NULL || from == NULL) return -1;

	struct user_node * user = find_user_node(to);
	if(!user) return -1;
	if(user->message_count == 0) return -1;

	struct message_node * message_queue_head = remove_message_queue_head(user);
	if(message_queue_head == NULL) return -1;

	if(copy_to_user(from, message_queue_head->from, strlen(message_queue_head->from)) > 0) return -1;
	if(copy_to_user(message, message_queue_head->message, strlen(message_queue_head->message)) > 0) return -1;

	kfree(message_queue_head->from);
	kfree(message_queue_head->message);
	kfree(message_queue_head);
